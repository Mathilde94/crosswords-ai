...
class WordsRepository:
    ...
    def build_from(words: List[str]) -> WordsRepository:
        path_crosswords_ai = os.path.dirname(os.path.realpath(__name__)).strip("/crosswords")
        # Alternative path you can create from:
        # cat nytcrosswords.csv| awk '{print $1}' |  awk -F','  '{print $2}' | sort | uniq > unique_words.txt
        file_path = "/{root}/fine_tuning/dataset/unique_words.txt".format(root=path_crosswords_ai)
        if os.path.isfile(file_path):
            with open(file_path, "r") as f:
                words = f.readlines()
            words = [w.lower().strip("\n") for w in words]
            words_with_frequency = [(w, zipf_frequency(w, 'en', wordlist='small')) for w in words]
            ...
            #     print(f, len(words))
            words = log_bin_frequencies[3] + log_bin_frequencies[4] + log_bin_frequencies[5]
