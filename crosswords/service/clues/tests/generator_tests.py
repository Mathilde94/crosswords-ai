...
class TestClueGenerator(unittest.TestCase):
    ...
    concepts = [
        Concept(c)
        for c in ["inheritance", "super", "composition", "class", "attributes"]
    ]
    ...
    def test_prepare_prompt(self):
        ...
        prompt = generator.prepare_prompt(
            self.word, self.title, self.section, self.concepts
        )
    ...
    @mock.patch(
        ...
        patch_get_clue_from_llm,
    )
    def test_execute(self):
        generator = ClueGenerator()
        clues = asyncio.run(generator.execute(self.concepts, self.title, self.section))
        self.assertEqual(
            [c.clue for c in clues],
            ["clue_for_{}".format(concept.word) for concept in self.concepts],
        )
